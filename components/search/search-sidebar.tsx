...
interface SidebarContentProps {
  filters: SearchSidebarProps["filters"]
  hasFilters: boolean
...
function SearchSidebarInner({
  ...
  const contentProps = { filters, hasFilters, updateFilter, clearAll }
    ...
    <>
      ...
      <div className="relative hidden lg:flex">
        ...
        <button
          ...
          className="absolute top-6 -right-8 z-10 flex flex-col items-center justify-center gap-1 bg-white border border-gray-200 shadow-sm rounded-r-xl px-2 py-4 hover:shadow-md transition-all duration-200 group"
        >
          {collapsed ? (
            <ChevronRight className="h-4 w-4 text-gray-400 group-hover:text-gray-700 transition-colors" />
          ) : (
            <ChevronLeft className="h-4 w-4 text-gray-400 group-hover:text-gray-700 transition-colors" />
          )}
          <span
      ...
      {/* Mobile — drawer overlay, controlled by parent */}
      <div className="lg:hidden">
        ...
        <div
          className={cn(
            "fixed top-0 left-0 z-50 h-full w-80 bg-white shadow-2xl overflow-y-auto transition-transform duration-300 ease-in-out",
...
export function SearchSidebar({
  ...
  return (
    <Suspense fallback={<div className="hidden lg:block w-72 border-r border-gray-100" />}>
      <SearchSidebarInner
        filters={filters}
